...
Each .txt file generated by the script begins with:

# COMMAND RAN: <the exact command executed>
# RUN AT: <ISO-8601 timestamp>
--------------------------------------------------

This makes it easy to see what was executed and to replay, debug, or modify commands later.

## Notes

• The script uses set -euo pipefail, but most enumeration commands end with "|| true",
  ensuring that a single failing tool does not stop the entire run.

• RustScan is invoked via "rust". If your system uses "rustscan" instead, update the
  rust_cmd variable in the script.

• HTTP enumeration automatically skips common WinRM ports (5985, 47001) to avoid noisy
  or irrelevant web scans.

## Disclaimer

This tool is intended for **legal penetration testing and education only**.
Do **not** use it on systems without explicit, written authorization.
You are solely responsible for how you choose to run this script.
